...
$SourceDocName = "\AdjustmentRule.xml"
...
$XLSDocName = "_WTKConfigurationTemplate_ADJ_.xlsx"
...
"<Request Action='RetrieveAllforUpdate'><AdjustmentRule/></Request></Kronos_WFC>"
...
$XMLPath = "/Kronos_WFC/Response/AdjustmentRule"
...
$RuleDetail = $XMLContent.root.Kronos_WFC.Response.AdjustmentRule
}
...
$RuleVersions = $Rule.RuleVersions.RuleVersion

foreach ($Version in $RuleVersions) {


    $EffDate = $Version.EffectiveDate
    $ExpDate = $Version.ExpirationDate
    $Description = $Version.Description

    if ($ExpDate -eq "1/01/3000") {

    $RuleWorkSheet.Columns.Item(1).Rows.Item($RowNumber) = $RuleName
    $RuleWorkSheet.Columns.Item(2).Rows.Item($RowNumber) = $Description
    ...
    $RuleWorkSheet.Columns.Item(4).Rows.Item($RowNumber).NumberFormat = "YYYY-MM-DD"
        ...
        $LabAccount = $Trigger.LaborAccountPattern

        $Attributes = $Trigger.AdjustmentAllocation.AdjustmentAllocation
        
        $AdjType = $Attributes.AdjustmentType
        $Type = $Attributes.Type
        $Amount = $Attributes.Amount
        $MatchAnywhere = "TRUE"
        $UseHighestWage = $Attributes.UseHighestWageSwitch
        $OverridePrimaryJob = $Attributes.OverrideIfHomeSwitch
        $BonusRate = $Attributes.BonusRateAmount
        $BonusHourlyRate = $Attributes.BonusRateHourlyRate
        $LaborAccountType = $Attributes.LaborAccountType
        $JoborLocation = ""
        $BonusPayCode = $Attributes.PayCode
        $TimePeriod = $Attributes.TimePeriod
        $OncePerDay = $Attributes.OncePerDay
        $TimeAmountMinimumTime = $Attributes.TimeAmountMinimumTime
        $TimeAmountMaximumTime = $Attributes.TimeAmountMaximumTime
        $TimeAmountMaximumAmount = $Attributes.TimeAmountMaximumAmount
        ...
        $PayCodes = $Trigger.PayCodes.SimpleValue
